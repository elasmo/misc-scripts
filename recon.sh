...
recon "Users"               "$(cat /etc/passwd | grep -v ^#)"
recon "Grousp"              "$(cat /etc/group | grep -v ^#)"
...
recon "Sudo permissions"    "$(sudo -ln 2> /dev/null|| echo "None")"
recon "Command history"     "$(cat ~/.bash_history || cat ~/.history)"
recon "Environment"         "$(env)"
recon "Shells"              "$(cat /etc/shells | grep -v ^#)"
...
recon "Suspected credentials in logs"       "$(grep -lE 'pass|crede|creds' /var/log/*.log 2> /dev/null || echo 'N/A')"
recon "Logs"                "$(find /var/log -type f -exec ls -la {} \; 2> /dev/null)"
recon "Open files"          "$(lsof -i -n 2> /dev/null || echo 'N/A')"
recon "root mail"           "$(head /var/mail/root 2> /dev/null || echo 'N/A')"
recon "Processes running as root" \
                            "$(ps auxw | grep root)"
