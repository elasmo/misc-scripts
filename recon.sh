...
recon "Surecon perms"          "$(surecon -ln 2> /dev/null|| echo "None")"
recon "Command history"     "$(history)"
#surecon -l 2>/dev/null | grep -w 'nmap|perl|awk|find|bash|sh|man|more|less|vi|vim|nc|netcat|python|ruby|lua|irb' | xargs -r ls -la 2>/dev/null
